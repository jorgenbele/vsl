...
/* Comment to make output match (exactly) with
 * expected symbol output. */
#define SHOW_SYM_LINE_COL
...
/* lookup_symb: Lookup a symbol by doing lookups on all scopes
 *              from top to down until it is found. */
...
void ir_bind_names(ir_ctx_t *ctx, symbol_t *function, node_t *root)
{
    ...
    for (uint64_t i = 0; i < params->n_children; i++) {
        ...
        /* Push the symbol to global vector, local vector
         * and function scope. */
        PUSH_SYMBOL(ctx, symb);
        IR_INSERT_FUNC_LOCAL_SYMBOL(function, symb);
    ...
    /* Destroy scope for this function. */
